 ...
 *
 * 修理屋で武器を修理・強化する純粋関数を提供する。
 *
 * 制限:
 *  - 各装備につき修理は1回のみ（WeaponInstance.repairedAtShop フラグで管理）
 *  - 各装備につき強化は1回のみ（WeaponInstance.upgradedAtShop フラグで管理）
 *
 * 修理オプション（25% / 50% / 全回復）+ 確率的な特殊イベント:
 *  5%  : 失敗 — 耐久値が修理予定量の半分だけ逆に減少（下限 1）
 * 60%  : 成功 — 通常通りに修理
 * 25%  : 好調 — 修理量 + 10% ボーナス
 * 10%  : 最高 — 修理量 + (atk+1 または maxDurability+5)
 *
 * 強化オプション（ATK+2 / MaxDUR+10）:
 *  費用 = maxDurability × 25 ゴールド（耐久なし武器は atk × 50）
 */

import type { GameState, WeaponInstance } from './game-state';
...
function upgradeCost(weapon: WeaponInstance): number {
  if (weapon.maxDurability !== null) {
    return weapon.maxDurability * 25;
  }
  return weapon.atk * 50;
}
